...
public class Dominion {
    ...
    public static void main(String[] args) {
        ...
        Map<Player, Integer> numWinsPerPlayer = new HashMap<>();
        Map<Player, Integer> numTiesPerPlayer = new HashMap<>();
        for (var p : players) {
            numWinsPerPlayer.put(p, 0);
            numTiesPerPlayer.put(p, 0);
        }
        ...
        for (int gameNum = 0; gameNum < numGamesToPlay; ++gameNum) {
            ...
            for (var p : players) {
                if (p.state.gameResult == GameResult.WIN) {
                    ...
                    numWinsPerPlayer.merge(p, 1, Integer::sum);
                } else if (p.state.gameResult == GameResult.TIE) {
                    // Merge here will increment the value at the key p
                    numTiesPerPlayer.merge(p, 1, Integer::sum);
                }
